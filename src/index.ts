...
class IntFormItem {
    ...
    validate(): boolean {
        let val = parseInt(this.#control.value);
...
class DecimalFormItem {
    ...
    validate(): boolean {
        let val = parseFloat(this.#control.value);
...
function main() {
    let form = document.querySelector('form')!; // we know the form exists
    let name = new StringFormItem(document.querySelector('#name') as HTMLInputElement, document.querySelector('#nameError')!);
    let quantity = new IntFormItem(document.querySelector('#quantity') as HTMLInputElement, document.querySelector('#quantityError')!);
    let price = new DecimalFormItem(document.querySelector('#price') as HTMLInputElement, document.querySelector('#priceError')!);
    let tax = new DecimalFormItem(document.querySelector('#tax') as HTMLInputElement, document.querySelector('#taxError')!);
    ...
    form.addEventListener('submit', (e) => {
        ...
        let subtotal = quantity.value * price.value;
        let total = subtotal + (subtotal * (tax.value/100.0))
        let result_str = `'${name.value}' x ${quantity.value} @ $${price.value.toFixed(2)} = ${subtotal.toFixed(2)} + ${tax.value}% tax = ${total.toFixed(2)}`;
